...
export default function HelpList() {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      if (editingRequest) {
        // Update
        const res = await helpApi.updateHelp(editingRequest.id, {
          text: formData.text,
        });
        const updated = requests.map((req) =>
          req.id === editingRequest.id ? res.data : req,
        );
        setRequests(updated);
        setFiltered(updated);
      } else {
  ...
  if (loading)
    return <div className="loading-spinner">Loading your inquiries...</div>;
            ...
            <li key={req.id} className="help-item">
              <div className="help-content">
                <div className="help-meta">
                  <span
                    className={`status-badge ${req.status?.toLowerCase() || "pending"}`}
                  >
                    {req.status || "PENDING"}
                  </span>
                  <span className="event-name">
                    Event:{" "}
                    {events.find((e) => e.id === req.eventId)?.name ||
                      req.eventId}
                  </span>
